...
import { PositionUtils, TimeUtils } from "../../utils";
import func from "./01";
...
function process(input: string) {
	return input.split("\n\n").map((text) => {
		...
		return {
			a: parse(A),
			b: parse(B),
			prize: PositionUtils.add(parse(P), [10000000000000, 10000000000000]),
		};
	});
}

const example = process(`Button A: X+94, Y+34
  Button B: X+22, Y+67
  Prize: X=8400, Y=5400
...
it("should pass example", () => {
	expect(TimeUtils.log(() => func(example))).toBe(875318608908);
});

const challenge = process(`Button A: X+19, Y+50
Button B: X+58, Y+35
...
it("should pass challenge", () => {
	expect(TimeUtils.log(() => func(challenge))).toBe(101406661266314);
});
