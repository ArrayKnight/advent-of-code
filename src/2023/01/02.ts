...
function digit(str: string, index: number) {
	const char = str[index];

	return /\d/.test(char) ? char : null;
}

function spelledOut(str: string, index: number) {
	const s = str.substring(index);

	return numbers.reduce((acc: number | null, word, i) => {
		if (acc != null) return acc;

		return s.startsWith(word) ? i + 1 : null;
	}, null);
}

function setValue(
	value: number | string | null,
	setters: Array<() => number | string | null>,
) {
...
console.log(
	input.split("\n").reduce((acc, i) => {
		...
		return acc + Number.parseInt(`${first ?? last ?? 0}${last ?? first ?? 0}`);
	}, 0),
);
