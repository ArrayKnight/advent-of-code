const input = `........897.......839...........651.399.............236...............................343...986...........308...............................
...
function getNumber(matrix: Matrix, x: number, y: number) {
  if (!/\d/.test(matrix[y][x])) return NaN

  let chars = matrix[y][x];
  let left = x - 1;
  let right = x + 1;

  while (/\d/.test(matrix[y][left])) {
    chars = matrix[y][left] + chars;
    matrix[y][left] = '.';
    left--;
  }

  while (/\d/.test(matrix[y][right])) {
    chars += matrix[y][right];
    matrix[y][right] = '.';
    right++;
  }
...
function getAdjacentNumbers(x: number, y: number) {
  ...
  const clone = [...matrix.map((line) => [...line])];

  return adjacent.reduce((acc: number[], coord) => {
    const num = getNumber(clone, ...coord);
...
console.log(matrix.reduce((acc, line, y) => {
  return acc + line.reduce((a, char, x) => {
    if(/\*/.test(char)) {
      const adjacent = getAdjacentNumbers(x, y);

      if (adjacent.length === 2) {
        return a + (adjacent[0] * adjacent[1]);
      }
    }
