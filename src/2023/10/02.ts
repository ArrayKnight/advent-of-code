const input = `FJ77.F7F.FF.F..7-J.7F|7-7-7-7FJJ-7J-LL-7FL.F-7.F-F77F7F7-77-F--7-FF|-F-L7-|.F7|7.F-77..-FF|--F.J7..|7FFL-JJ7-F-..J77.|FFJ77L---J7F7.J-F77JJ.
...
const map = input.split('\n').reduce((acc: string[][], line, index) => {
  if (index > 0) {
    acc.push(new Array(line.length * 2 - 1).fill(' '));
  }

  acc.push(line.split('').reduce((a: string[], char, i) => {
    if (i > 0) {
      a.push(' ');
    }
...
const start = map.reduce((acc: Point | null, line, y) => {
  if (acc) return acc;
...
function is(a: Point, b: Point) {
  return a.every((v, i) => v === b[i]);
}

function translate(point: Point, delta: Delta): Point {
  const [x, y] = point;
...
const left: Delta = [-2, 0];
const right: Delta = [2, 0];
...
function connects(map: string[][], node: Point, next: Point) {
  const aConnectsToB = is(node, start) || pipes[map[node[1]]?.[node[0]]]?.some((delta) => is(translate(node, delta), next));
  const bConnectsToA = is(next, start) || pipes[map[next[1]]?.[next[0]]]?.some((delta) => is(translate(next, delta), node));

  return aConnectsToB && bConnectsToA;
}
...
while (true) {
  ...
  for (let i = 0; i < adjacent.length; i++) {
    ...
    if (connects(map, node, next)) {
      const [x, y] = translate(node, [(next[0] - node[0]) / 2, (next[1] - node[1]) / 2]);
...
function checkOutside(x: number, y: number) {
  const char = map[y]?.[x];

  if (!char || char === '+' || char === 'O') return;
  ...
  if (map[y][x] !== 'O' && adjacent.some((node) => map[node[1]]?.[node[0]] === 'O')) {
    map[y][x] = 'O';
  }
...
for (let i = 0; i < map[0].length; i++) {
  checkOutside(i, 0);
  checkOutside(i, map.length - 1);
}

for (let i = 0; i < map.length; i++) {
  checkOutside(0, i);
  checkOutside(map[0].length - 1, i);
}

for (let i = 0; i < check.length; i++) {
  checkOutside(...check[i])
}

console.log(map.flat().join('').replace(/[O+ ]/g, '').length);
