const input = `Game 1: 9 red, 5 blue, 6 green; 6 red, 13 blue; 2 blue, 7 green, 5 red
...
function game(str: string) {
  const reg = /(\d+) (blue|green|red)/g;
  const game = {blue: 0, green: 0, red: 0};
  let arr;

  while (arr = reg.exec(str)) {
    const [, count, color] = arr;

    game[color] = Math.max(game[color], parseInt(count));
  }

  return game;
}

const games = input.split('\n').reduce((acc: Games, line) => {
  const key = /Game (\d+):/.exec(line)?.[1] ?? '';
...
function isPossible(game: Game) {
  return Object.entries(game).every(([key, num]) => num <= maxes[key])
}

console.log(Object.entries(games).reduce((acc, [key, game]) => acc + (isPossible(game) ? parseInt(key) : 0), 0));
