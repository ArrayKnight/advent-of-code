const input = `vm{a>1183:zz,s>1553:A,s<827:xc,hls}
...
const workflows = workflowsRaw.split('\n').reduce((acc: Workflows, line) => {
  const [id, instructions] = line.split(/[{}]/);
  const reg = /([xmas])([<>])(\d+):(?:(?:([AR]|[a-z]{2,3}),([AR]|[a-z]{2,3}))|([AR]|[a-z]{2,3}))/g;
  const workflows: Workflow[] = [];
  let arr;

  while (arr = reg.exec(instructions)) {
    const [,prop, operator, value, truthy, falsy, truthyToo] = arr;
    const righthand = parseInt(value);

    workflows.push({
      id,
      prop,
      gt: operator === '>' ? righthand : undefined,
      lt: operator === '<' ? righthand : undefined,
      truthy: truthy ?? truthyToo,
      falsy,
    });
  }
...
const parts = partsRaw.split('\n').map((line) => {
  const pairs = line.substring(1, line.length - 1).split(',');
...
console.log(parts.reduce((acc, part) => {
  let key = 'in';

  while (key !== 'A' && key !== 'R') {
    const rules = workflows[key];

    for (let i = 0; i < rules.length; i++) {
      const rule = rules[i];

      if (
        (rule.gt !== undefined && part[rule.prop] > rule.gt) ||
        (rule.lt !== undefined && part[rule.prop] < rule.lt)
      ) {
        key = rule.truthy;
        break;
      } else if (rule.falsy) {
